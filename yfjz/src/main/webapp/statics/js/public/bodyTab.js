...
layui.define(["element","jquery"],function(exports){
    ...
    Tab.prototype.navBar = function(strData){
        ...
        for(var i=0;i<data.length;i++){
            ...
            if(data[i].list != undefined && data[i].list.length > 0){
                ...
                ulHtml += '<cite style="font-weight:bold">'+data[i].name+'</cite>';
                ulHtml += '<span class="layui-nav-more"></span>';
                ulHtml += '</a>';
                ulHtml += '<dl class="layui-nav-child">';
                for(var j=0;j<data[i].list.length;j++){
                	if(JSON.stringify(data[i].list[j])=="{}"){
                		continue;
                	}
                    if(data[i].list[j].target == "_blank"){
                        ulHtml += '<dd><a data-url="'+data[i].list[j].url+'" target="'+data[i].list[j].target+'">';
                    }else{
                    ...
                    /*if(data[i].list[j].icon != undefined && data[i].list[j].icon != ''){
                        if(data[i].list[j].icon.indexOf("icon-") != -1){
                            ulHtml += '<i class="seraph '+data[i].list[j].icon+'" data-icon="'+data[i].list[j].icon+'"></i>';
                        }else{
                            ulHtml += '<i class="layui-icon" data-icon="'+data[i].list[j].icon+'">'+data[i].list[j].icon+'</i>';
                        }
                    }*/
                    if(data[i].list[j].icon != undefined && data[i].list[j].icon != ''){
                        ulHtml += '<i class="'+data[i].list[j].icon+'"></i>';
                    }
                    if(!data[i].list[j].url){
                      ulHtml += '<cite style="color:#CCCCCC">'+data[i].list[j].name+'</cite></a></dd>';
					}else{
	...
	Tab.prototype.tabAdd = function(_this){
		...
		var that = this;
		var opt=_this.attr("class");
        var openTabNum = that.tabConfig.openTabNum;
        tabFilter = that.tabConfig.tabFilter;
		if(!_this.attr("data-url")&&opt==undefined){
			layer.msg("您无权使用该模块！");
			return;
		}
