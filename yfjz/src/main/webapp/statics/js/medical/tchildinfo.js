...
$(function () {
    $("#jqGrid").jqGrid({
        ...
        onSelectRow:function(rowid,iRow,iCol,e){
            ...
            // alert(vm.sequenceNoId);
            $("#precheck_iframe").attr("src", "../medical/tchildprecheck.html?chilCode="+rowid+"&chilName="+rowData.chilName+"&sequenceNoId="+vm.sequenceNoId);
            $("#inoculation_iframe").attr("src", "../medical/tchildinoculate.html?chilCode="+rowid+"&birthday="+rowData.chilBirthday);
            $("#move_iframe").attr("src", "../medical/tchildmove.html?chilCode="+rowid+"&chilHere="+rowData.chilHere+"&chilName="+rowData.chilName);
            $("#abnormal_iframe").attr("src", "../medical/tchildabnormal.html?chilCode="+rowid+"&chilName="+rowData.chilName);
            $("#taboo_iframe").attr("src", "../medical/tchildtaboo.html?chilCode="+rowid+"&chilName="+rowData.chilName);
            $("#allergy_iframe").attr("src", "../medical/tchildallergy.html?chilCode="+rowid+"&chilName="+rowData.chilName);
            $("#infection_iframe").attr("src", "../medical/tchildinfection.html?chilCode="+rowid+"&chilName="+rowData.chilName);
            $("#cancel_iframe").attr("src", "../bus/tbuscancel.html?chilCode="+rowid+"&chilName="+rowData.chilName);

            setchildName=rowData.chilName;
            setchildCode=rowData.chilCode;
            vm.childCode=setchildCode;
            vm.childName=setchildName;
    ...
    methods: {
        ...
        saveOrUpdate: function (event) {
            ...
            if ($("#chilForm").data('bootstrapValidator').isValid()) {//获取验证结果，如果成功，执行下面代码
                ...
                // if ($("#detailed").val() != null && $("#detailed").val().length < 18) {
                ...
                // }
                // if ($("#detailed_home").val() != null && $("#detailed").val().length < 18) {
        ...
        loadCallChild: function () {
            ...
            $("#jqGrid").jqGrid('setGridParam', {
                url: '../child/list?page=1&chilCode='+queuechilCode,
            }).trigger("reloadGrid");
        ...
        complite : function () {
            ...
            var param ={
                "sequenceNoId" : vm.sequenceNoId,
                "childCode":vm.childCode,
                "childName":vm.childName
            };
            console.log(vm.chilCode);
            $.ajax({
                type:"post",
                url:"../tchildprecheck/precheck?sequenceNoId="+vm.sequenceNoId+"&childCode="+setchildCode+"&childName="+setchildName,
                dataType:'json',
...
//监听预检台完成事件
vm.$on("precheckcomplite",function(queue){
    if(vm.childCode==undefined||vm.childCode==null||vm.childCode==""){
        layer.msg("请选择一个儿童");
        return;
    }
    vm.sequenceNoId = queue.id;
    vm.complite();
});
